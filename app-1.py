...
def fetch_coin_info(coin_id: str):
    ...
    for attempt in range(3):
        ...
        if r.status_code == 429:
            wait = int(r.headers.get("Retry-After", 15)) if attempt == 0 else 30
            st.warning(f"⏳ CoinGecko rate limit — {wait}s wait kar raha hoon... (attempt {attempt+1}/3)")
            time.sleep(wait)
            continue
        r.raise_for_status()
        return r.json()
    raise Exception("CoinGecko rate limit: 3 attempts ke baad bhi 429 — thodi der baad try karo")
